...
const BLOOD_GROUPS = [...STOCK_ORDER];
...
function stableSeedNumber(input) {
...
function normalizeSeedDonors(rawList) {
  ...
  const normalized = rawList
    .map((raw, index) => {
      ...
      const blood = String(raw?.blood || raw?.bloodGroup || "").trim().toUpperCase();
      const lat = Number(raw?.lat ?? raw?.location?.lat);
      const lng = Number(raw?.lng ?? raw?.location?.lng);

      if (!name || !blood || !Number.isFinite(lat) || !Number.isFinite(lng)) {
        return null;
      }

      const idToken = raw?.id ?? `${index + 1}`;
      const seed = stableSeedNumber(idToken);
      const available = raw?.available !== undefined ? Boolean(raw.available) : true;
      const eta = 8 + (seed % 18);
      const donationFrequency = 1 + (seed % 5);
      const responseRate = Number((available ? 0.78 : 0.6) + ((seed % 20) / 100)).toFixed(2);
      const lastDonationDaysAgo = daysSince(raw?.lastDonation);

      return {
        id: `donor-${String(idToken).replace(/[^a-zA-Z0-9_-]/g, "")}`,
        userId: null,
        name,
        blood,
        lat,
        ...
        donationFrequency,
        lastDonationDaysAgo: Math.max(0, lastDonationDaysAgo)
      };
    })
    .filter(Boolean);

  return normalized.length > 0 ? normalized : FALLBACK_DONORS;
}
...
try {
  ...
  INITIAL_DONORS = normalizeSeedDonors(parsed);
} catch {
  INITIAL_DONORS = FALLBACK_DONORS;
}
...
export function createDataStore(databasePath) {
  ...
  function recordDemandEvent(event) {
    const createdAt = event.createdAt || nowIso();
    const id = event.id || crypto.randomUUID();

    db.prepare(`
      INSERT INTO demand_events (
        id, blood, units, urgency, status, location, lat, lng, created_by, created_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
  ...
  function listDemandEvents({ since } = {}) {
    const sinceValue = since || new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
    return db
      .prepare("SELECT * FROM demand_events WHERE created_at >= ? ORDER BY created_at ASC")
      .all(sinceValue)
      .map((row) => ({
        id: row.id,
        blood: row.blood,
  ...
  function createIncident(incident) {
    ...
    db.prepare(`
      INSERT INTO incidents (
        id, title, severity, radius_km, location, lat, lng,
  ...
  function listIncidents({ activeOnly = true } = {}) {
    const now = nowIso();
    const rows = activeOnly
      ? db.prepare("SELECT * FROM incidents WHERE starts_at <= ? AND ends_at >= ? ORDER BY severity DESC, ends_at ASC").all(now, now)
      : db.prepare("SELECT * FROM incidents ORDER BY starts_at DESC").all();
